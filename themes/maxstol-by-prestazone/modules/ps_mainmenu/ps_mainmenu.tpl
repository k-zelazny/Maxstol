{function name="desktopMenu" nodes=[] depth=0 parent=null}
  {if $nodes|count}
    <ul
      ...
      class="{if $depth === 0}main-menu__tree h-100{else}submenu{/if}"
      data-depth="{$depth}"
    >
    {foreach from=$nodes item=node}
      ...
      >
        ...
        {/if}
          <a
            class="main-menu__tree__link{if $node.children|count} dropdown-toggle{/if}{if $depth > 0} dropdown-item{/if} caption"
            href="{$node.url}"
            data-depth="{$depth}"
    ...
    >
      ...
      <ul class="menu__list">
        ...
        {foreach from=$nodes item=node}
          ...
          >
            <a
              class="{if $depth>= 0}menu__link{/if} caption"
              href="{$node.url}"
              data-depth="{$depth}"
              {if $node.open_in_new_window}target="_blank"{/if}
            >
