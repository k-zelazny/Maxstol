...
final class CategoryTreeProvider
{
  ...
  public function getMainCategoriesWithSubcategories(Context $context): array
  {
    ...
    $tree = \Category::getNestedCategories(
      $homeCategoryId,
      $langId,
      true,
      null,
    ...
    foreach ($mainCategoryNodes as $mainCategoryNode) {
      if (!isset($mainCategoryNode['children']) || !is_array($mainCategoryNode['children'])) {
        continue;
      }
      foreach ($mainCategoryNode['children'] as $subCategoryNode) {
    ...
    foreach ($mainCategoryNodes as $mainCategoryNode) {
      ...
      foreach ($subCategoryNodes as $subCategoryNode) {
        ...
        $linkRewrite = (string) ($subCategoryNode['link_rewrite'] ?? '');
        
        $subcategories[] = [
          'id_category' => $subCategoryId,
          'name' => (string) ($subCategoryNode['name'] ?? ''),
          'link' => $context->link->getCategoryLink($subCategoryId, $linkRewrite),
  ...
  private function buildIntList(array $values): string
  {
    ...
    $sanitized = array_filter($sanitized, static function (int $value): bool {
      return $value > 0;
    });
